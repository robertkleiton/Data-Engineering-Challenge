...
from (
	select
		distinct
		"num_exec",
		upper(md5(trim(trim(upper(coalesce("location", '-1'))) || trim(upper(coalesce("indicator", '-1'))) || trim(coalesce("time", '-1')))))::text as "cod_key_table",
		...
		dense_rank() over (partition by upper(md5(trim(trim(upper(coalesce("location", '-1'))) || trim(upper(coalesce("indicator", '-1'))) || trim(coalesce("time", '-1')))))::text order by "num_exec" desc) as rank
