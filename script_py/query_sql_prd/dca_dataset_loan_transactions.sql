...
from (
	select
		distinct
		"num_exec",
		upper(md5(trim("transaction report id")))::text as "cod_key_table",
		...
		dense_rank() over (partition by upper(md5(trim("transaction report id")))::text order by "num_exec" desc) as rank
	from
