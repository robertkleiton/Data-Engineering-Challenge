...
from (
	select
		distinct
		"num_exec",
		upper(md5(trim(trim(upper("credit agreement type")) || trim(upper("guarantee number")) || trim("guarantee start date") || trim("guarantee start date"))))::text as "cod_key_table",
		...
		dense_rank() over (partition by upper(md5(trim(trim(upper("credit agreement type")) || trim(upper("guarantee number")) || trim("guarantee start date") || trim("guarantee start date"))))::text order by "num_exec" desc) as rank
