...
from (
	select
		distinct
		"num_exec",
		upper(md5(trim(trim(upper("country_code")) || trim(upper("indicator_code")) || trim("year"))))::text as "cod_key_table",
		...
		case when "year" = '' then null else "year"::int4 end as "desc_year",
		...
		dense_rank() over (partition by upper(md5(trim(trim(upper("country_code")) || trim(upper("indicator_code")) || trim("year"))))::text order by "num_exec" desc) as rank
