insert into aux.dca_dataset_utilization_and_claims(
	...
	"final date for placng loans under coverage",
	"fiscal year",
	"guarantee end date",
	"guarantee number",
	...
	select
		...
		,case
			...
			when
				...
				coalesce(stg."final date for placng loans under coverage",'-1')     <> coalesce(aux."final date for placng loans under coverage", '-1') or
